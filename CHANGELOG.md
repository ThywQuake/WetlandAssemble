...
- Restored the missing Phase 4 per-dataset trend contract path: `src/WA/comparison/trend_contract.py` now writes and semantically reloads one `trend_surface` NetCDF plus one `trend_regional_summary` CSV per `dataset_id + region_id`, while `src/WA/comparison/trends.py` and `scripts/run_phase4_trend_contract.py` now persist explicit region/dataset/requested-window checkpoints before agreement so wide reruns can resume at the dataset boundary instead of recomputing every trend input.
- Added `scripts/submit_phase4_trend_contract.sh`, which fans out one region per SLURM job, requires an explicit `--repo`, hardcodes `--no-skip` into each generated trend-contract command, writes one summary TSV per fanout run, and makes the participant dataset list explicit in every job script.
- Restored the missing Phase 4 classification contract path: `src/WA/comparison/classification_contract.py` now rewrites region-scoped classification surfaces, summaries, and hotspot manifest/CSV pairs from the real Phase 3.6 global disagreement outputs plus the Phase 3.7 hotspot source trio, while `scripts/run_phase4_classification_contract.py` exposes the same `--region` / `--subset canonical` / `--subset ten` orchestration shape and fail-closed skip behavior as the other contract-aware runners.
- Added semantic reload helpers in `src/WA/visualization/phase4.py` for classification summaries and classification hotspot tables, plus focused regression coverage for malformed Phase 3.6/3.7 source failures and the new runner's help surface.
