#!/usr/bin/env python3
# ruff: noqa: E402
"""Batch-plot Phase 3.7 regional classification disagreement panels."""

from __future__ import annotations

import argparse
import logging
import sys
import tempfile
from collections.abc import Sequence
from pathlib import Path

import xarray as xr
import yaml

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "src"))

from WA.visualization.phase37 import (  # noqa: E402
    DEFAULT_PHASE37_SAMPLE_STEP,
    DEFAULT_PHASE37_SOURCE_LAT_CHUNK_SIZE,
    build_phase37_global_plot_dataset,
    plot_phase37_global_figure,
    subset_phase37_plot_dataset_to_bbox,
    write_phase37_global_plot_cache,
)

DEFAULT_PHASE37_INPUT_DIR = Path("results/phase3.6")
DEFAULT_PHASE37_OUTPUT_DIR = Path("results/figures/phase3.7_regions")
DEFAULT_PHASE37_CACHE_DIR = Path("results/cache/phase3_7")
DEFAULT_PHASE37_REGIONS_FILE = Path("config/priority_regions.yaml")
DEFAULT_PHASE37_YEAR = 2016
DEFAULT_PHASE37_DPI = 300

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(message)s",
)
logger = logging.getLogger(__name__)


class Phase37Region:
    """One named Phase 3.7 plotting region."""

    def __init__(
        self,
        *,
        region_id: str,
        label: str,
        bbox: tuple[float, float, float, float],
    ) -> None:
        self.region_id = region_id
        self.label = label
        self.bbox = bbox


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "按 priority_regions.yaml 批量绘制 Phase 3.7 regional disagreement panels，"
            "复用 500m 原网格 sparse-sampled cache。"
        ),
    )
    parser.add_argument(
        "--metrics-path",
        type=Path,
        help="显式指定 phase3_6_entropy_*.nc 路径；若不传则按 input-dir/year 推断",
    )
    parser.add_argument(
        "--classes-path",
        type=Path,
        help="显式指定 phase3_6_unified_classes_*.nc 路径；若不传则按 input-dir/year 推断",
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=DEFAULT_PHASE37_INPUT_DIR,
        help="Phase 3.6 NetCDF 输出目录 (默认：results/phase3.6)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=DEFAULT_PHASE37_OUTPUT_DIR,
        help="区域 PNG 输出目录 (默认：results/figures/phase3.7_regions)",
    )
    parser.add_argument(
        "--regions-file",
        type=Path,
        default=DEFAULT_PHASE37_REGIONS_FILE,
        help="区域配置文件 (默认：config/priority_regions.yaml)",
    )
    parser.add_argument(
        "--regions",
        nargs="*",
        help="可选：只绘制指定 region id 列表；默认绘制配置中的全部区域",
    )
    parser.add_argument(
        "--cache-dir",
        type=Path,
        default=DEFAULT_PHASE37_CACHE_DIR,
        help="Phase 3.7 sparse cache 目录 (默认：results/cache/phase3_7)",
    )
    parser.add_argument(
        "--cache-path",
        type=Path,
        help="显式指定 Phase 3.7 sparse cache 路径；若不传则按 year/sample-step 自动命名",
    )
    parser.add_argument(
        "--year",
        type=int,
        default=DEFAULT_PHASE37_YEAR,
        help="目标年份 (默认：2016)",
    )
    parser.add_argument(
        "--sample-step",
        type=int,
        default=DEFAULT_PHASE37_SAMPLE_STEP,
        help="500m 原网格稀疏采样步长；例如 8 表示每 8 个像元取 1 个 (默认：8)",
    )
    parser.add_argument(
        "--source-lat-chunk-size",
        type=int,
        default=DEFAULT_PHASE37_SOURCE_LAT_CHUNK_SIZE,
        help="构建 sparse cache 时的源文件纬向条带行数 (默认：512)",
    )
    parser.add_argument(
        "--dpi",
        type=int,
        default=DEFAULT_PHASE37_DPI,
        help="输出 PNG dpi (默认：300)",
    )
    parser.add_argument(
        "--no-prefer-cache",
        action="store_false",
        dest="prefer_cache",
        help="即使 sparse cache 已存在，也强制重建",
    )
    parser.add_argument(
        "--no-write-cache",
        action="store_false",
        dest="write_cache",
        help="不把 sparse cache 写回磁盘，仅本次内存构图",
    )
    return parser.parse_args(argv)


def main(argv: Sequence[str] | None = None) -> int:
    args = parse_args(argv)
    metrics_path = args.metrics_path or default_metrics_path(args.input_dir, year=args.year)
    classes_path = args.classes_path or default_classes_path(args.input_dir, year=args.year)
    cache_path = args.cache_path or default_cache_path(
        args.cache_dir,
        year=args.year,
        sample_step=args.sample_step,
    )

    if not metrics_path.is_file():
        logger.error("metrics 文件不存在：%s", metrics_path)
        return 1
    if not classes_path.is_file():
        logger.error("classes 文件不存在：%s", classes_path)
        return 1

    regions = load_phase37_regions(args.regions_file)
    if args.regions:
        wanted = set(args.regions)
        regions = [region for region in regions if region.region_id in wanted]

    if not regions:
        logger.error("没有可绘制的 region")
        return 1

    logger.info(
        "Phase3.7 regional plotting args: metrics=%s classes=%s cache=%s output=%s "
        "sample_step=%s regions=%s prefer_cache=%s write_cache=%s dpi=%s",
        metrics_path,
        classes_path,
        cache_path,
        args.output_dir,
        args.sample_step,
        len(regions),
        args.prefer_cache,
        args.write_cache,
        args.dpi,
    )

    plot_dataset: xr.Dataset | None = None
    try:
        plot_dataset = load_or_build_plot_dataset(
            metrics_path,
            classes_path,
            cache_path=cache_path,
            sample_step=args.sample_step,
            source_lat_chunk_size=args.source_lat_chunk_size,
            prefer_cache=args.prefer_cache,
            write_cache=args.write_cache,
        )

        args.output_dir.mkdir(parents=True, exist_ok=True)
        for region in regions:
            logger.info("Phase3.7 region start: %s", region.region_id)
            regional_plot = subset_phase37_plot_dataset_to_bbox(plot_dataset, region.bbox)
            output_path = default_region_output_path(
                args.output_dir,
                region_id=region.region_id,
                year=args.year,
                sample_step=args.sample_step,
            )
            suptitle = (
                f"Phase 3.7 Regional Classification Disagreement ({args.year})\n"
                f"{region.label} • 500m grid sparse-sampled every {args.sample_step} pixel(s)"
            )
            plot_phase37_global_figure(
                regional_plot,
                output_path=output_path,
                dpi=args.dpi,
                suptitle=suptitle,
            )
            logger.info("Phase3.7 region done: %s -> %s", region.region_id, output_path)
    finally:
        if plot_dataset is not None:
            plot_dataset.close()

    logger.info("Phase3.7 regional plotting finished: %d region(s)", len(regions))
    return 0


def load_or_build_plot_dataset(
    metrics_path: Path,
    classes_path: Path,
    *,
    cache_path: Path,
    sample_step: int,
    source_lat_chunk_size: int,
    prefer_cache: bool,
    write_cache: bool,
) -> xr.Dataset:
    """Load the sparse plot dataset, optionally materializing the cache on disk."""

    plot_dataset: xr.Dataset | None = None
    if prefer_cache and cache_path.is_file():
        logger.info("Phase3.7 regional cache hit: %s", cache_path)
        plot_dataset = xr.open_dataset(cache_path)
        loaded = plot_dataset.load()
        plot_dataset.close()
        return loaded

    if write_cache:
        logger.info("Phase3.7 regional cache miss: %s", cache_path)
        write_phase37_global_plot_cache(
            metrics_path,
            classes_path,
            cache_path=cache_path,
            sample_step=sample_step,
            source_lat_chunk_size=source_lat_chunk_size,
        )
        plot_dataset = xr.open_dataset(cache_path)
        loaded = plot_dataset.load()
        plot_dataset.close()
        return loaded

    logger.info("Phase3.7 regional cache disabled: building plot dataset in memory")
    with tempfile.TemporaryDirectory(prefix="phase37-regional-plot-") as _temp_dir:
        return build_phase37_global_plot_dataset(
            metrics_path,
            classes_path,
            sample_step=sample_step,
        )


def load_phase37_regions(regions_file: Path) -> list[Phase37Region]:
    """Load plotting regions from `config/priority_regions.yaml`."""

    document = yaml.safe_load(regions_file.read_text(encoding="utf-8")) or {}
    regions = document.get("regions")
    if not isinstance(regions, dict):
        raise ValueError("regions_file must contain a top-level 'regions' mapping")

    ordered: list[tuple[int, str, dict[str, object]]] = []
    for region_id, payload in regions.items():
        if not isinstance(payload, dict):
            raise ValueError(f"Region {region_id!r} must be a mapping")
        ordered.append((int(payload.get("priority", 9999)), str(region_id), payload))

    result: list[Phase37Region] = []
    for _priority, region_id, payload in sorted(ordered):
        bbox = payload.get("bbox")
        if not isinstance(bbox, list) or len(bbox) != 4:
            raise ValueError(f"Region {region_id!r} must provide bbox as 4-item list")
        result.append(
            Phase37Region(
                region_id=region_id,
                label=str(payload.get("label", region_id)),
                bbox=(float(bbox[0]), float(bbox[1]), float(bbox[2]), float(bbox[3])),
            )
        )
    return result


def default_metrics_path(input_dir: Path, *, year: int) -> Path:
    return input_dir / f"phase3_6_entropy_global_500m_{year}.nc"


def default_classes_path(input_dir: Path, *, year: int) -> Path:
    return input_dir / f"phase3_6_unified_classes_global_500m_{year}.nc"


def default_cache_path(cache_dir: Path, *, year: int, sample_step: int) -> Path:
    return cache_dir / f"phase3_7_global_plot_cache_global_500m_{year}_sample{sample_step}.nc"


def default_region_output_path(
    output_dir: Path,
    *,
    region_id: str,
    year: int,
    sample_step: int,
) -> Path:
    return output_dir / f"phase3_7_region_{region_id}_global_500m_{year}_sample{sample_step}.png"


if __name__ == "__main__":
    raise SystemExit(main())
