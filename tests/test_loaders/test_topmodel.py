...
def test_topmodel_loader_open_time_series_keeps_config_and_forcing_dims(
    tmp_path: Path,
) -> None:
    base_path = tmp_path / "topmodel"
    template = xr.Dataset(
    ...
    write_netcdf(base_path / "cfg_a/force_a/fwet_cfg_a_force_a_reso025_2020.nc", template)
    write_netcdf(base_path / "cfg_b/force_b/fwet_cfg_b_force_b_reso025_2020.nc", template)

    loader = get_loader("topmodel", with_common_fields(base_path, loader_type="topmodel"))
    result = loader.open_time_series(time_range=("2020-01-01", "2020-02-29"))
    try:
        assert result.sizes["config"] == 2
        assert result.sizes["forcing"] == 2
